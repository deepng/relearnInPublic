...
describe('Basic Bofore Test', () => {
  ...
  it('login to vymo', () => {
    cy.fixture('users').then((user) => {
      cy.visit(user.stagingUrl).wait(6000)
      cy.get('#field-username').should('exist').type(user.validUserId);
      cy.get('button[data-test-id="login-button"]').should('exist').click();
      cy.get('#field-password').should('exist').type(user.validUserPassword);
